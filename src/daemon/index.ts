...
const DAEMON_VERSION = "0.7.17";

/** Lex-compare dotted versions ("0.7.5" vs "0.7.17"). Returns negative/0/positive
...
async function main(): Promise<void> {
  ...
  server.register("meta.handshake", async (params, ctx) => {
    // Register caller identity if provided. Pre-0.7.17 clients send empty
    // params and stay anonymous (still counted in activeClients but absent
    // from the per-client registry). 0.7.17+ clients send {clientInfo}.
