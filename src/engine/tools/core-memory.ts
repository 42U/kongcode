          ...
          case "add": {
            ...
            if (tier === 0) {
              ...
              if (fit.dropped.some((d) => d.id === NEW_ID)) {
                ...
                return {
                  content: [{ type: "text" as const, text:
                    `Tier 0 budget full: ${fit.usedChars}/${budgetChars} chars across ${existing.length} entries. ` +
              ...
              if (evicted.length) {
                ...
                return {
                  content: [{ type: "text" as const, text:
                    `Refused: adding this entry (p${candidate.priority}) would push ${evicted.length} lower-priority ` +
