...
export class WedgeDetector {
  private timeouts = 0;
  private futileHeals = 0;
  private streakStartedAt: number | null = null;
  private escalatedFlag = false;

  constructor(
    private readonly cfg: { minTimeouts: number; minHeals: number; minStreakMs: number } = WEDGE_DEFAULTS,
  ...
  recordDeadlineTimeout(): boolean {
    if (this.streakStartedAt === null) this.streakStartedAt = this.now();
    this.timeouts++;
    return this.check();
  }
  ...
  recordHealCompleted(): boolean {
    if (this.streakStartedAt === null) return false;
    this.futileHeals++;
    return this.check();
  }
  ...
  private check(): boolean {
    if (this.escalatedFlag || this.streakStartedAt === null) return false;
    if (this.timeouts < this.cfg.minTimeouts) return false;
    if (this.futileHeals < this.cfg.minHeals) return false;
    if (this.now() - this.streakStartedAt < this.cfg.minStreakMs) return false;
...
export class SurrealStore {
    ...
    this.reconnecting = (async () => {
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        try {
          ...
          if (this.wedge.recordHealCompleted()) this.fireIrrecoverableWedge();
          return;
        } catch (e) {
          if (attempt < MAX_ATTEMPTS) {
            await new Promise((r) => setTimeout(r, BACKOFF_MS[attempt - 1]));
  ...
  private readonly wedge = new WedgeDetector();
  ...
  private onIrrecoverableWedge: ((info: string) => void) | null = null;

  setIrrecoverableWedgeHandler(cb: (info: string) => void): void {
    this.onIrrecoverableWedge = cb;
  }

  private fireIrrecoverableWedge(): void {
    if (this.shutdownFlag) return;
    const info =
      `SurrealDB self-heal is not working: ${this.wedge.stats()} — every reconnect built a ` +
  ...
  private async deadlineQuery(
    ...
    try {
      ...
      if (flagZombie && e instanceof Error && e.message.includes("deadline exceeded")) {
        ...
        if (this.wedge.recordDeadlineTimeout()) this.fireIrrecoverableWedge();
      }
      throw e;
    }
  }
   ...
   */
  async getAllCoreMemory(tier?: number, sessionId?: string): Promise<CoreMemoryEntry[]> {
    try {
      if (tier === 1 && sessionId) {
