 ...
 */
export function deriveTranscriptPath(sessionId: string, cwd: string, home: string): string {
  if (!sessionId || !cwd || !home) return "";
...
function readTail(path: string, bytes: number): string | null {
  try {
    ...
    if (stats.size <= bytes) return readFileSync(path, "utf-8");
 ...
 */
export function countToolCallsSinceText(
  transcriptPath: string,
  minTextChars: number = MIN_NARRATION_CHARS,
): number {
  ...
  for (const line of raw.split(/\r?\n/)) {
    ...
    if (obj.type === "user") {
      if (!isPurelyToolResult(content)) count = 0;
      continue;
    }
    if (obj.type !== "assistant") continue;
    ...
    for (const b of blocks) {
      ...
      if (type === "text") {
        ...
        if (t.trim().length >= minTextChars) count = 0;
      } else if (type === "tool_use") {
        count++;
      }
    }
    // A string-content assistant entry is plain text.
    if (typeof content === "string" && content.trim().length >= minTextChars) count = 0;
  }
  return count;
}
...
function isPurelyToolResult(content: unknown): boolean {
