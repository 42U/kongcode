...
export function applyCoreBudgetVerbose(entries, budgetChars) {
    const render = (text, cap) => text.length > cap ? text.slice(0, cap) + "..." : text;
    const cost = (text, cap) => render(text, cap).length + 6;
    ...
    for (const s of slots) {
        ...
        if (target <= s.cap || s.e.text.length <= s.cap)
            continue;
...
function applyCoreBudget(entries, budgetChars) {
    ...
    if (r.dropped.length) {
        log.warn(`[core-memory] budget exceeded: ${r.usedChars}/${r.budgetChars} chars, ` +
...
async function formatContextMessage(nodes, store, session, skillContext = "", tier0Entries = [], tier1Entries = []) {
    ...
    if (directives.length > 0) {
        ...
        const directiveLines = directives.map(d => `  [${d.priority}] ${d.type} → ${stripStructuralTags(String(d.target))}: ${stripStructuralTags(String(d.instruction))}`);
