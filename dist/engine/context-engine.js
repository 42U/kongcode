...
export class KongCodeContextEngine {
    ...
    async assemble(params) {
        ...
        if (systemPromptSection)
            additions.push(systemPromptSection);
        ...
        if (graduation) {
            let graduationBlock = "[SOUL GRADUATION — CELEBRATE WITH THE USER]\n" +
        ...
        if (session._hasMigratableFiles) {
            additions.push("[MIGRATION AVAILABLE] This workspace has files from the default context engine " +
        ...
        for (const section of additions) { // additions are already in priority order
            if (spaTotalChars + section.length > SPA_BUDGET_CHARS && keptAdditions.length > 0)
                break;
            ...
            if (role === "user" || role === "assistant") {
                ...
                if (role === "user") {
                    ...
                    if (worthEmbedding && session.currentConfig) {
                        const predicted = predictQueries(text, (session.currentConfig.intent ?? "general"));
    ...
    async afterTurn(params) {
        ...
        if (tokensSinceCleanup >= session.midSessionCleanupThreshold) {
            ...
            cleanupOps.push(checkACANReadiness(store, this.state.config.thresholds.acanTrainingThreshold)
                .catch(e => swallow("midCleanup:acan", e)));
            cleanupOps.push((async () => {
                const transition = await checkStageTransition(store);
                if (transition.transitioned) {
                    log.info(`[MATURITY] Stage transition: ${transition.previousStage ?? "nascent"} → ${transition.currentStage}. ` +
