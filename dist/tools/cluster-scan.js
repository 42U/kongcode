...
async function fetchNeighborConcepts(state, resultIds) {
    ...
    try {
        for (const rid of resultIds) {
            const rows = await state.store.queryFirst(`SELECT VALUE ->mentions->concept AS out FROM ${rid}
         UNION SELECT VALUE ->about_concept->concept AS out FROM ${rid}
         UNION SELECT VALUE ->artifact_mentions->concept AS out FROM ${rid}`).catch(() => []);
            // Different traversal shape — just grab concept ids from any returned rows
            const contents = [];
            for (const r of rows) {
                const id = r.out ?? r.id;
                if (id)
                    contents.push(String(id));
            }
            if (contents.length > 0)
                neighborMap.set(rid, contents);
        }
    }
    catch (e) {
